...
-- Crear un procedimiento almacenado
 ...
 CREATE PROCEDURE obtener_escuela
AS
BEGIN
    SELECT a.ID_AfilxEmpresa, a.Numero_documento_afil AS Documento, n.numero_documento_NET AS Documento_NET, 
        'AfilxEmpresa' AS Fuente 
...
CREATE PROCEDURE obtener_afiliados_no_en_nueva_net
AS
BEGIN
    SELECT a.ID_AfilxEmpresa, a.Numero_documento_afil AS Documento
    FROM AfilxEmpresa a
    INNER JOIN Nueva_NET n ON a.ID_AfilxEmpresa = n.id_afilxempresa_net
    EXCEPT
    SELECT n.id_afilxempresa_net AS ID_AfilxEmpresa, n.numero_documento_NET AS Documento
    FROM Nueva_NET n;
END;

CREATE PROCEDURE combinar_afiliados_y_grabados
AS
BEGIN
    SELECT a.ID_AfilxEmpresa, a.Numero_documento_afil AS Documento, 'AfilxEmpresa' AS Fuente
    FROM AfilxEmpresa a
    INNER JOIN Grabado_fechas g ON a.Numero_documento_afil = g.Numero_documento_Grab
    UNION
    SELECT g.IDNuevaNET_Grab AS ID_AfilxEmpresa, g.Numero_documento_Grab AS Documento, 'Grabado_fechas' AS Fuente
    FROM Grabado_fechas g;
END;

CREATE PROCEDURE obtener_nueva_net_no_en_grabado
AS
BEGIN
    SELECT n.id_afilxempresa_net AS ID_AfilxEmpresa, n.numero_documento_NET AS Documento
    FROM Nueva_NET n
    INNER JOIN Grabado_fechas g ON n.NuevaNet_id = g.IDNuevaNET_Grab
    EXCEPT
    SELECT g.IDNuevaNET_Grab AS ID_AfilxEmpresa, g.Numero_documento_Grab AS Documento
    FROM Grabado_fechas g;
END;
...
BEGIN
    ...
    FROM AfilxEmpresa a
    INNER JOIN Nueva_NET n ON a.ID_AfilxEmpresa = n.id_afilxempresa_net
    WHERE a.Numero_documento_afil = @documento
    UNION ALL
    SELECT a.ID_AfilxEmpresa, a.Numero_documento_afil AS Documento, g.Numero_documento_Grab AS Documento_Grab, 
        'Grabado_fechas' AS Fuente 
    FROM AfilxEmpresa a
    ...
    FROM Nueva_NET n
    INNER JOIN Grabado_fechas g ON n.NuevaNet_id = g.IDNuevaNET_Grab
    WHERE n.numero_documento_NET = @documento;
END;
